...
# Variáveis métricas - pega a descritiva das variaveis
print(dados[['Age','AnnualSalary']].describe())

# Variáveis categóricas - pega as frequencia
...
dados = pd.get_dummies(dados, 
                       columns=['Gender'], 
                       drop_first=True, #apaga o primeiro(male ou female)
                       dtype='int')

#%% Separando as variáveis Y e X

X = dados.drop(columns=['Purchased']) #variaveis para a x - features
y = dados['Purchased'] #variavel target
...
rf_grid_model = GridSearchCV(estimator = rf_grid, 
                             ...
                             scoring='accuracy',
                             cv=5, verbose=2) #cv - cross validation 5 fold
